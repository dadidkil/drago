...
**Что это.** SMTP — сервер исходящей почты, через который сайт отправляет письма от имени `noreply@dragotop.ru`:
приглашения бойцам, ссылки для сброса пароля, уведомления на email. Сайт сам письма не доставляет — он передаёт их
почтовому серверу по логину и паролю служебного ящика.

**Пока SMTP не настроен:** приглашение можно выдать вручную (карточка пользователя → «Показать ссылку-приглашение»),
сброс пароля — `make reset-link email=…` на сервере; письма-уведомления не уходят (в кабинете и Telegram — уходят).

**Вариант 1 — почта ISPmanager** (бесплатно, ящики бойцов создаются из админки). Нужны PTR и открытый порт 25
от хостера (12.1), почтовый домен и DNS-записи (12.2). Затем:
...
2. В `.env`:
   ...
   ```
   `mail.dragotop.ru` должен иметь сертификат (12.2, шаг 1): приложение требует шифрование и проверяет сертификат.
3. `make deploy` → `/admin/integrations` → «Отправить тестовое письмо себе».

**Вариант 2 — Яндекс 360 для бизнеса или VK WorkSpace**, если хостер не открывает порт 25 или письма с сервера
попадают в спам. Подключите домен у провайдера (DNS — [dns-records.md](dns-records.md), раздел 2B), создайте ящик
`noreply@dragotop.ru`, включите для него доступ почтовых программ и создайте **пароль приложения**. В `.env`:

| | Яндекс 360 | VK WorkSpace (Mail.ru) |
|---|---|---|
| `SMTP_HOST` | `smtp.yandex.ru` | `smtp.mail.ru` |
| `SMTP_PORT` | `465` | `465` |
| `SMTP_SECURE` | `true` | `true` |
| `SMTP_USER` | `noreply@dragotop.ru` | `noreply@dragotop.ru` |
| `SMTP_PASSWORD` | пароль приложения | пароль приложения |

Одновременно MX домена может указывать только на одного провайдера: выбрали вариант 2 — ящики бойцов тоже будут у
него (в `.env` `MAIL_PROVIDER=manual`, создаются в админке провайдера). Тарифы уточняйте на сайте провайдера.

Почта считается рабочей только после проверки 12.6: DNS, PTR, SPF, DKIM, DMARC и доставка на Gmail, Яндекс и Mail.ru
и обратно.
...
6. Проверка: напишите сообществу «Начать» — придёт меню; «Вступить» → текст о наборе и ссылка на приложение МосРСО.
