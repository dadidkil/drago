...
Если свободно меньше 8 ГБ, скрипт сначала сам очищает кэш сборки Docker — только кэш, данные не затрагиваются.
После деплоя кэш ужимается до 4 ГБ: на диске 30 ГБ без этого он разрастается до 15 ГБ и больше.
...
| Браузер: «неподдерживаемый протокол», `ERR_SSL_VERSION_OR_CIPHER_MISMATCH` | у сайта в ISPmanager нет сертификата / SSL не включён, либо DNS вашего провайдера ещё отдаёт старый IP (до 6 ч после смены записи; 25.09 так вёл себя Яндекс.DNS 77.88.8.8) | на своём ПК: `nslookup dragotop.ru` — если не 2.56.90.240, это кэш: подождать, или `ipconfig /flushdns` + в браузере «Безопасный DNS» (Cloudflare/Google), или проверить с мобильного интернета. Если IP верный — шаг 5.2: выпустить Let's Encrypt, включить SSL у сайта; затем `make nginx-proxy` |
